 ...
 * @param <K> This is the unique identifer that gets hashed
 * @param <V> This is the value associated to the key
 */
public class HashTable<K, V> {
     ...
     * It is missing the value to be returned?
    ...
    public String toString(int limit) {
        ...
        for (int i = 0; i < storage.length && count < limit; i++) {
            if (storage[i] != null && storage[i].isActive) {
                sb.append(String.format("%d: %s[", i, storage[i].key)).append(storage[i].value).append("]\n");
                count++;
            }
        }
        return sb.toString();
